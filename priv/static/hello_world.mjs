...
// build/dev/javascript/hello_world/hello_world.mjs
var FILEPATH = "src\\hello_world.gleam";
...
function view_count(count) {
  return p(
    ...
    toList([text3("Count: "), text3(to_string(count))])
  );
}
function view(model) {
  return div(
    toList([]),
    toList([
      view_button(new UserClickedDecrement(), "-1"),
      view_count(model),
      view_button(new UserClickedIncrement(), "+1")
    ])
  );
}
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      { value: $, start: 368, end: 417, pattern_start: 379, pattern_end: 384 }
    );
  }
  return void 0;
}
